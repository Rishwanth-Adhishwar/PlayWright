...
```ts
  ...
  5  |     const a = 30;
  6  |     const b = 20;
  7  |     await page.locator("#sum1").fill(a.toString());
  8  |     await page.locator("#sum2").fill(b.toString());
  9  |     console.log("First value:", await page.locator("#sum1").inputValue());
  10 |     console.log("Second value:", await page.locator("#sum2").inputValue());
  11 |     await expect(page.getByRole("button", { name: "Get Sum" })).toBeEnabled();
  12 |     await page.getByRole("button", { name: "Get Sum" }).click();
> 13 |     await expect(page.locator("#addmessage")).toHaveText((a + b).toString());
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  14 |     const existing = await page.locator("#addmessage").innerText();
  15 |     console.log("Result:", existing);
  16 | });
```