...
- main:
  ...
  - textbox "Please enter your Message": Rishwanth
...
```ts
  ...
  8  |     const existing1 = await page.getAttribute('(//input[@type="text"])[3]', 'placeholder');
  9  |     await page.getByRole('button', { name: "Get Checked Value" }).click();
> 10 |     await expect(page.locator('#message')).toContainText("Rishwanth");
     |                                            ^ Error: expect(locator).toContainText(expected) failed
  11 |     const actual = await page.locator("#message").textContent();
  12 |     console.log("Updated Name: ", actual);
  13 | });
```