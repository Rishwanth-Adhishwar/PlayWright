...
- Name: promptalert.test.ts >> @smoke prompt alert
- Location: tests\promptalert.test.ts:3:1
...
Call log:
  ...
  - waiting for locator('//p[@id="prompt-demo"]')
    10 × locator resolved to <p id="prompt-demo" class="ml-10 mb-20 mt-20"></p>
       - unexpected value ""
...
```ts
  ...
  2  | 
  3  | test('@smoke prompt alert', async ({ page }) => {
  4  |     await page.goto("https://www.testmuai.com/selenium-playground/javascript-alert-box-demo/");
  5  |     page.on('dialog', async diaolg => {
  6  |         console.log("Allert Message: ", diaolg.message());
  7  |         await diaolg.accept("Rishwanth");
  8  |     });
  9  |     await page.getByRole('button', { name: 'Click Me' }).nth(2).click();
> 10 |     expect(page.locator('//p[@id="prompt-demo"]')).toContainText("Rishwanth");
     |                                                    ^ Error: expect(locator).toContainText(expected) failed
  11 | 
  12 | 
  13 | })
