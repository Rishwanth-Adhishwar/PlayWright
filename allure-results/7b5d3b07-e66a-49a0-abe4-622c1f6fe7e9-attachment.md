...
```ts
  ...
  10 |     const boxing = await page.getByRole("button", { name: "Get Sum" }).boundingBox();
  11 |     console.log("Boxing: ", boxing);
  12 |     await page.getByRole("button", { name: "Get Sum" }).isEnabled();
  13 |     await page.getByRole("button", { name: "Get Sum" }).click();
  14 |     const existing = await page.locator("//p[@id='addmessage']").innerText();
  15 |     console.log("Result: ", existing);
  16 |     const actual = a + b;
> 17 |     await expect(existing).toEqual(actual.toString());
     |                            ^ Error: expect(received).toEqual(expected) // deep equality
  18 | });
```