...
```ts
  ...
  8  |     const b = 20;
  9  | 
  10 |     await page.locator("#sum1").fill(a.toString());
  11 |     await page.locator("#sum2").fill(b.toString());
  12 | 
  13 |     console.log("First value:", await page.locator("#sum1").inputValue());
  14 |     console.log("Second value:", await page.locator("#sum2").inputValue());
  15 | 
  16 |     const getSumBtn = page.locator("//button[text()='Get Sum']");
  17 | 
  18 |     await expect(getSumBtn).toBeVisible();
  19 |     await expect(getSumBtn).toBeEnabled();
  20 | 
  21 |     await getSumBtn.scrollIntoViewIfNeeded();
  22 |     await getSumBtn.click({ force: true });
  23 | 
> 24 |     await expect(page.locator("#addmessage")).toHaveText((a + b).toString());
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  25 | 
  26 |     console.log("Result:", await page.locator("#addmessage").innerText());
  27 | });
```