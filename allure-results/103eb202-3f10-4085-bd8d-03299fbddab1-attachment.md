...
Expected: "Welcomsadmin"
Received: "Welcome admin"
```
...
> 13 |     expect.soft(welcomeText).toBe("Welcomsadmin");
     |                              ^ Error: expect(received).toBe(expected) // Object.is equality
  14 | });
  15 | 
  16 | test.skip("Login Test 2", async ({ page }) => {
  ...
  26 |     expect(welcomeText).toBe("Welcome admin");
  27 | });
  28 | 
  ...
  39 |     expect(welcomeText).toBe("Welcome admin");
  40 | });
```