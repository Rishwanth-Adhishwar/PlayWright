...
```ts
  ...
  6  |     const a = "30", b = "20";
  7  |     await page.getByPlaceholder('Please enter first value').fill(a);
  8  |     await page.getByPlaceholder('Please enter second value').fill(b);
  ...
  13 |     const actual = a + b;
> 14 |     await expect(existing).toEqual(actual);
     |                            ^ Error: expect(received).toEqual(expected) // deep equality
  15 | });
```